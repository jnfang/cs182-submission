################################################################################################################
# midi.py       Version 1.7     06-Dec-2014     David Johnson, Bill Manaris, Kenneth Hanson
...
class MidiIn(Receiver):
   ...
   def openInputDevice(self, selectedItem):
      ...
      self.transmitter = self.midiDevice.getTransmitter()
      self.transmitter.setReceiver(self)
      ...
      if isinstance(message, ShortMessage):     # is this a ShortMessage?
         ...
         if msgType != 15:  # is it a channel message?
            ...
            if self.showIncomingMessages:   # echo print incoming MIDI messages?
               print "MIDI In - Event Type:", eventType, ", Channel:", msgChannel, ", Data 1:", msgData1, ", Data 2:", msgData2
...
def _stopActiveMidiObjects_():
   ...
   # first, stop MidiIn objects
   for midiIn in _ActiveMidiInObjects_:
      #midiIn.midiInput.stop()
      midiIn.midiInput.close()

   # now, stop MidiOut objects
   for midiOut in _ActiveMidiOutObjects_:
      midiOut.midiOutput.close()
